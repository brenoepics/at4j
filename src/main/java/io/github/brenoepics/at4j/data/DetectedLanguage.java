...
public class DetectedLanguage {
   ...
   * @param isTranslationSupported A boolean indicating if the language is
   *     supported for translation.
   * @param isTransliterationSupported A boolean indicating if the language is
  ...
  public static DetectedLanguage ofJSON(String baseText, ObjectNode jsonNode) {
    if (jsonNode == null || !jsonNode.has("language") || !jsonNode.has("score"))
      return null;

    DetectedLanguage detected =
        new DetectedLanguage(baseText, jsonNode.get("language").asText(),
                             jsonNode.get("score").floatValue());

    if (jsonNode.has("isTranslationSupported")) {
      detected.isTranslationSupported =
          jsonNode.get("isTranslationSupported").asBoolean();
    }

    if (jsonNode.has("isTransliterationSupported")) {
      detected.isTransliterationSupported =
          jsonNode.get("isTransliterationSupported").asBoolean();
    }
  ...
  /**
   * Gets A float value indicating the confidence in the result. The score is
   * <b>between zero and one</b>, and a low score indicates low confidence.
   *
   ...
   */
  public boolean isTranslationSupported() { return isTranslationSupported; }

  /**
   * Gets a boolean indicating if the language is supported for transliteration.
   *
   * @return A boolean indicating if the language is supported for
   *     transliteration.
   */
  ...
  public String toString() {
    return "DetectedLanguage{"
        + "language='" + languageCode + '\'' + ", score=" + score +
