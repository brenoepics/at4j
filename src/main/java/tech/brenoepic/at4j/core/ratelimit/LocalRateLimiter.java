 ...
 *
 * <p>To compensate for misalignment with the real global ratelimit bucket, it is recommended to not
 * set the value to the exact global ratelimit but a lower value. If the global ratelimit for your
 * bot is 50 requests / 1 second (the default), you should choose a value that prevents overlapping
 * with the real bucket. This can be achieved with the following rules:
 *
 * <ul>
 *   <li>Choose an {@code amount} that is between {@code 1} and half of the real amount (in this
 *       example {@code 1} - {@code 25}).
 *   <li>Calculate the {@code bucketDuration} using {@code (amount * realDuration) / (realAmount -
 *       amount)}.
 * </ul>
 *
 * For the 50 requests / 1 second ratelimit, these rules allow the following ratelimits (only a
 * subset):
 *
 * <ul>
 *   <li>About {@code 1} request per {@code 21 ms} ({@code amount = 1} and {@code bucketDuration =
 *       Duration.ofMillis((long) Math.ceil(1000D / 49D))})
 *   <li>About {@code 5} request per {@code 112 ms} ({@code amount = 5} and {@code bucketDuration =
 *       Duration.ofMillis((long) Math.ceil(5000D / 45D))}).
 *   <li>{@code 10} request per {@code 250 ms} ({@code amount = 10} and {@code bucketDuration =
 *       Duration.ofMillis((long) Math.ceil(10000D / 40D))}).
 *   <li>{@code 25} request per {@code 1 sec} ({@code amount = 25} and {@code bucketDuration =
 *       Duration.ofMillis((long) Math.ceil(25000D / 25D))}).
 * </ul>
 *
 * Choosing a lower {@code amount} increases the maximum throughput but can limit your ability to
 * perform actions in bulk.
 *
 * @see <a
 *     href="https://docs.microsoft.com/en-us/azure/cognitive-services/translator/request-limits">Azure
 *     RateLimit Docs</a>
 */
