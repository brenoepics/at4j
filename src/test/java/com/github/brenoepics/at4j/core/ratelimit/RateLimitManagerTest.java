...
class RateLimitManagerTest<T> {
		...
		@Mock
		private RateLimitBucket<T> bucket;
		...
		void doesNotHandleResponseWhenResultIsNull() {
				assertDoesNotThrow(() -> rateLimitManager.handleResponse(request, null, bucket, System.currentTimeMillis()));
		}
		...
		void queuesRequestWhenBucketIsNotPresent() {
				...
				rateLimitManager.queueRequest(request);
				assertDoesNotThrow(() -> rateLimitManager.handleResponse(request, result, bucket, System.currentTimeMillis()));
		}
		...
		@Test
		void handleResponseDoesNotUpdateBucketInformationWhenResponseCodeIsNot429AndRequestResultIsDone() {
				when(result.getResponse()).thenReturn(response);
				when(response.code()).thenReturn(200);
				when(request.getResult()).thenReturn(CompletableFuture.completedFuture(result));
